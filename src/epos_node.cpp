...
void *deviceHandle = ePos.deviceOpen("USB0", 1);					// Open USB communication	
...
class EposNode : public rclcpp::Node
{
  public:
    ...
    void init_epos()
    {
      ...
      ePos.nodeMotion(deviceHandle, 1, 10);				
      ePos.nodeMotion(deviceHandle, 2, 20); 						
      ePos.nodeMotion(deviceHandle, 3, 30); 			
    }
  ...
  private:
    ...
    void topic_callback(const example_interfaces::msg::Float64MultiArray::SharedPtr msg) const
    {
      ...
      // moveAllFingers(msg->data[0], msg->data[1], msg->data[2]);
      ePos.nodeMotion(deviceHandle, 1, msg->data[0]);
      ePos.nodeMotion(deviceHandle, 2, msg->data[1]);
      ePos.nodeMotion(deviceHandle, 3, msg->data[2]);
